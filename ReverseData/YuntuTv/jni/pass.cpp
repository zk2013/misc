...
typedef void (*fun_ff_make_absolute_url)(char *buf, int size, const char *base,
                          const char *rel);
fun_ff_make_absolute_url old_ff_make_absolute_url = NULL;
void hook_ff_make_absolute_url(char *buf, int size, const char *base,
                          const char *rel) 
{
             if (old_ff_make_absolute_url)  
                    old_ff_make_absolute_url(buf,size,base,rel);
             LOGI("buf=%s",buf);
             LOGI("base=%s",base);
             LOGI("rel=%s",rel);
}
                      
typedef int  (*fun_ffp_prepare_async_l)(void *ffp, const char *file_name);
fun_ffp_prepare_async_l old_ffp_prepare_async_l = NULL;

int    hook_ffp_prepare_async_l(void *ffp, const char *file_name) {
		LOGI("%s\n",file_name);
		if (old_ffp_prepare_async_l) {
			return old_ffp_prepare_async_l(ffp, file_name);
		}
		return 0;
}
...
{	
	...
	char libName[] = "libfungo_player.so";
    ...
    char libName2[] = "libfungo_ffmpeg.so";
    
	HookFun(libName,"ffp_prepare_async_l", (void *)hook_ffp_prepare_async_l, (void **)&old_ffp_prepare_async_l);
    HookFun(libName2,"ff_make_absolute_url", (void *)hook_ff_make_absolute_url, (void **)&old_ff_make_absolute_url);
